...
@Component({
  selector: 'app-fiat-transactions',
  templateUrl: './fiat-transactions.component.html',
  styleUrls: ['./fiat-transactions.component.scss'],
  providers: [AdvancedService, DecimalPipe]
})
export class FiatTransactionsComponent implements OnInit {
  ...
  async getFiatTransactions() {
    ...
    const query = await firebase.firestore().collection("local-transactions").where("created_by.user_id", "==", uid).orderBy("timestamp", "desc").get();
